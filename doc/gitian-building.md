...
*Setup instructions for a gitian build of MNS using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MNS
...
Clone the git repositories for mnscoin and gitian and then checkout the mnscoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/mnscoin-crypto/mnscoin.git
cd mnscoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the mnscoin repository
...
To build MNS (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the mnscoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mnscoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/mnscoin-crypto/mnscoin
...
For example:
```bash
URL=https://github.com/crowning-/mnscoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit mnscoin=${COMMIT} --url mnscoin=${URL} ../mnscoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mnscoin=${COMMIT} --url mnscoin=${URL} ../mnscoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mnscoin=${COMMIT} --url mnscoin=${URL} ../mnscoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mnscoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mnscoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/mnscoin-build.assert
```
...
[mnscoin/gitian.sigs](https://github.com/mnscoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
