...
#if defined(NDEBUG)
#error "MNS cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in umnscoin) are considered zero fee (for relaying and mining)
...
{
    RenameThread("mnscoin-scriptch");
    scriptcheckqueue.Thread();
}

bool RecalculateMNSSupply(int nHeightStart)
{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MNS: We use certain sporks during IBD, so check to see if they are
