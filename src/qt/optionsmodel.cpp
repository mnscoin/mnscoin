...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MNS);
    ...
    if (!settings.contains("nAnonymizeMNSAmount"))
        settings.setValue("nAnonymizeMNSAmount", 1000);

    nAnonymizeMNSAmount = settings.value("nAnonymizeMNSAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMNSAmount"))
        SoftSetArg("-anonymizemnscoinamount", settings.value("nAnonymizeMNSAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mnscoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMNSAmount:
            return QVariant(nAnonymizeMNSAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeMNSAmount:
            nAnonymizeMNSAmount = value.toInt();
            settings.setValue("nAnonymizeMNSAmount", nAnonymizeMNSAmount);
            emit anonymizeMNSAmountChanged(nAnonymizeMNSAmount);
            break;
        case CoinControlFeatures:
