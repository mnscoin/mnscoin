...
# operating on affine and Mnscoinobian coordinates, including the point at infinity
...
class mnscoinobianpoint:
  def __init__(self, x, y, z, infinity=0):
    ...
    self.Infinity = infinity
  def __str__(self):
    return "mnscoinobianpoint(X=%s,Y=%s,Z=%s,inf=%s)" % (self.X, self.Y, self.Z, self.Infinity)


def point_at_infinity():
  return mnscoinobianpoint(1, 1, 1, 1)


def negate(p):
  if p.__class__ == affinepoint:
    return affinepoint(p.x, -p.y)
  if p.__class__ == mnscoinobianpoint:
    return mnscoinobianpoint(p.X, -p.Y, p.Z)
  assert(False)
...
def law_mnscoinobian_weierstrass_add(A, B, pa, pb, pA, pB, pC):
  """Check whether the passed set of coordinates is a valid Mnscoinobian add, given assumptions"""
  assumeLaw = (good_affine_point(pa) +
               good_affine_point(pb) +
               good_mnscoinobian_point(pA) +
               good_mnscoinobian_point(pB) +
...
def law_mnscoinobian_weierstrass_double(A, B, pa, pb, pA, pB, pC):
  """Check whether the passed set of coordinates is a valid Mnscoinobian doubling, given assumptions"""
  assumeLaw = (good_affine_point(pa) +
               good_affine_point(pb) +
               good_mnscoinobian_point(pA) +
               good_mnscoinobian_point(pB) +
...
def law_mnscoinobian_weierstrass_add_opposites(A, B, pa, pb, pA, pB, pC):
  assumeLaw = (good_affine_point(pa) +
               good_affine_point(pb) +
               good_mnscoinobian_point(pA) +
               good_mnscoinobian_point(pB) +
...
def law_mnscoinobian_weierstrass_add_infinite_a(A, B, pa, pb, pA, pB, pC):
  assumeLaw = (good_affine_point(pa) +
               good_affine_point(pb) +
               good_mnscoinobian_point(pA) +
               good_mnscoinobian_point(pB) +
...
def law_mnscoinobian_weierstrass_add_infinite_b(A, B, pa, pb, pA, pB, pC):
  assumeLaw = (good_affine_point(pa) +
               good_affine_point(pb) +
               good_mnscoinobian_point(pA) +
               good_mnscoinobian_point(pB) +
...
def law_mnscoinobian_weierstrass_add_infinite_ab(A, B, pa, pb, pA, pB, pC):
  assumeLaw = (good_affine_point(pa) +
               good_affine_point(pb) +
               good_mnscoinobian_point(pA) +
               good_mnscoinobian_point(pB) +
...
laws_mnscoinobian_weierstrass = {
  'add': law_mnscoinobian_weierstrass_add,
  'double': law_mnscoinobian_weierstrass_double,
  'add_opposite': law_mnscoinobian_weierstrass_add_opposites,
  'add_infinite_a': law_mnscoinobian_weierstrass_add_infinite_a,
  'add_infinite_b': law_mnscoinobian_weierstrass_add_infinite_b,
  'add_infinite_ab': law_mnscoinobian_weierstrass_add_infinite_ab
}


def check_exhaustive_mnscoinobian_weierstrass(name, A, B, branches, formula, p):
  """Verify an implementation of addition of Mnscoinobian points on a Weierstrass curve, by executing and validating the result for every possible addition in a prime field"""
  F = Integers(p)
    ...
    for zb in xrange(1, p):
      for pa in points:
        for pb in points:
          for ia in xrange(2):
            for ib in xrange(2):
              pA = mnscoinobianpoint(pa.x * F(za)^2, pa.y * F(za)^3, F(za), ia)
              pB = mnscoinobianpoint(pb.x * F(zb)^2, pb.y * F(zb)^3, F(zb), ib)
              for branch in xrange(0, branches):
                ...
                if r:
                  match = False
                  for key in laws_mnscoinobian_weierstrass:
                    assumeLaw, require = laws_mnscoinobian_weierstrass[key](A, B, pa, pb, pA, pB, pC)
...
def check_symbolic_mnscoinobian_weierstrass(name, A, B, branches, formula):
  """Verify an implementation of addition of Mnscoinobian points on a Weierstrass curve symbolically"""
  ...
  pA = mnscoinobianpoint(ax * Az^2, ay * Az^3, Az, Ai)
  pB = mnscoinobianpoint(bx * Bz^2, by * Bz^3, Bz, Bi)
  ...
  for branch in xrange(branches):
    ...
    for key in laws_mnscoinobian_weierstrass:
      res[key].append((check_symbolic_function(R, assumeFormula, assumeBranch, laws_mnscoinobian_weierstrass[key], A, B, pa, pb, pA, pB, pC), branch))
