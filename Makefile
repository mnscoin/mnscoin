...
pkgdatadir = $(datadir)/mnscoin
pkgincludedir = $(includedir)/mnscoin
pkglibdir = $(libdir)/mnscoin
pkglibexecdir = $(libexecdir)/mnscoin
...
CONFIG_HEADER = $(top_builddir)/src/config/mnscoin-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/mnscoin-config.h.in \
...
ACLOCAL = ${SHELL} /prj/coinFactory/works/bitcoin/mnscoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /prj/coinFactory/works/bitcoin/mnscoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /prj/coinFactory/works/bitcoin/mnscoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /prj/coinFactory/works/bitcoin/mnscoin/build-aux/missing automake-1.15
AVX2_CXXFLAGS = -mavx -mavx2
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = mnscoin-cli
BITCOIN_DAEMON_NAME = mnscoind
BITCOIN_GUI_NAME = mnscoin-qt
BITCOIN_TX_NAME = mnscoin-tx
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /prj/coinFactory/works/bitcoin/mnscoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = mnscoin
PACKAGE_BUGREPORT = mnscoin.io
PACKAGE_NAME = Mnscoin Core
PACKAGE_STRING = Mnscoin Core 1.0.0
PACKAGE_TARNAME = mnscoin
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /prj/coinFactory/works/bitcoin/mnscoin
abs_srcdir = /prj/coinFactory/works/bitcoin/mnscoin
abs_top_builddir = /prj/coinFactory/works/bitcoin/mnscoin
abs_top_srcdir = /prj/coinFactory/works/bitcoin/mnscoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /prj/coinFactory/works/bitcoin/mnscoin/build-aux/install-sh
...
DIST_CONTRIB = $(top_srcdir)/contrib/mnscoin-cli.bash-completion \
	       $(top_srcdir)/contrib/mnscoin-tx.bash-completion \
	       $(top_srcdir)/contrib/mnscoind.bash-completion \
...
COVERAGE_INFO = baseline.info \
  test_mnscoin_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_mnscoin_coverage.info test_mnscoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/MnsCoin-Core $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/mnscoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/mnscoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/mnscoin-config.h
$(top_srcdir)/src/config/mnscoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/mnscoin-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/MnsCoin-Core: all-recursive
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/MnsCoin-Core
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/MnsCoin-Core: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_mnscoin.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_mnscoin -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_mnscoin_filtered.info: test_mnscoin.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_mnscoin_filtered.info
...
#test_mnscoin_coverage.info: baseline_filtered.info test_mnscoin_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_mnscoin_filtered.info -o $@

#total_coverage.info: test_mnscoin_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_mnscoin_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_mnscoin.coverage/.dirstamp:  test_mnscoin_coverage.info
...
#cov: test_mnscoin.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_mnscoin.coverage/ total.coverage/ test/tmp/ cache/ $(OSX_APP)
